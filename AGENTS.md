...
NEVER edit generated code. The server's `lib/src/generated/` directory and the whole `open_transport_map_client` package are rewritten by the code generator. Change the `.spy.yaml` models, the endpoints, or `lib/server.dart` instead.
...
The app is launched from `open_transport_map_flutter/lib/driver.dart`, which starts the Flutter driver extension with text entry emulation turned off so the app stays usable by hand. To let the driver type, set `enableTextEntryEmulation: true` there and `hot_restart` the app.

## What the app is

Open Transport Map shows live Västtrafik vehicle positions on a map.

The server relays the upstream Västtrafik SSE feed through the streaming endpoint `vehiclePositions.watchVehicles`. The stream is viewport-scoped: the client passes north/south/east/west bounds and the server subscribes to the upstream feed for that area. Each SSE event becomes one `VehicleDelta` with `updated` and `removed` vehicle ids. Line metadata (short name and colors) is fetched from the map-lines API once per subscription and attached to each vehicle.

The Flutter app renders the feed in a MapLibre map. Below street-level zoom (13.5) it draws a clustered GeoJSON source: cluster bubbles with counts, and single dots carrying the line short name. Above that zoom the cluster and dot layers hide via `maxzoom` and up to 50 animated arrow symbols take over, nearest to the viewport center first. Each symbol interpolates along the segment from its previous to its current position, with a duration derived from the reported speed, driven by a `Ticker`.

Sign-in is set up on the server (email identity provider, JWT) but not switched on in the UI. The app's `SignInScreen` can be wrapped around the map screen in `open_transport_map_flutter/lib/main.dart` to require authentication.
